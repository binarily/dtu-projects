...
struct port
{
	...
	struct message* message_list; //List of messages to relay
	uint8_t exists;
	
	struct thread* thread_list; //List of threads to unsuspend
	uint8_t is_receive; //Is the element on a list a receiving one?
};

//List of ports
extern struct port ports[MAX_THREADS];
...
int allocate_port(uint32_t, struct process*);
int find_port(uint32_t, struct process*);
int send(uint32_t, struct message*);
int receive(uint32_t, struct message*, uint32_t*);